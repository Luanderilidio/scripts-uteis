...
def add_logo_to_images(folder_path, logo_path, output_folder):
    ...
    redimensionando as imagens para o formato de story (1080x1920 pixels) e mantendo a qualidade.
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path) and filename.lower().endswith(image_extensions):
            try:
                ...
                # Redimensionamento para story (mantendo proporção)
                image.thumbnail(STORY_SIZE, Image.Resampling.LANCZOS)
                # Criar um fundo branco no tamanho de story
                story_image = Image.new("RGBA", STORY_SIZE, (255, 255, 255, 255))
                # Centralizar a imagem no fundo branco
                offset = ((STORY_SIZE[0] - image.size[0]) // 2, (STORY_SIZE[1] - image.size[1]) // 2)
                story_image.paste(image, offset)
                ...
                story_image.paste(resized_logo, position, resized_logo)
                
                # Converter para RGB e salvar
                output_path = os.path.join(output_folder, filename)
                story_image.convert("RGB").save(output_path, "JPEG", quality=95)
...
# Exemplo de uso
add_logo_to_images(
    folder_path="F:\\04 - Fim",
    logo_path="LOGO.png",
    output_folder="F:\\04 - Fim - LOGO"
)
