...
def add_logo_to_images(folder_path, logo_path, output_folder):
    ...
    FEED_SIZE = (1080, 1080)  # Resolução de feed no Instagram
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path) and filename.lower().endswith(image_extensions):
            try:
                ...
                # Redimensionamento com zoom para feed
                ratio = max(FEED_SIZE[0] / image.size[0], FEED_SIZE[1] / image.size[1])
                new_size = (int(image.size[0] * ratio), int(image.size[1] * ratio))
                image = image.resize(new_size, Image.Resampling.LANCZOS)

                # Cortar para o tamanho exato do feed (object-fit: cover; object-position: center)
                left = (image.size[0] - FEED_SIZE[0]) / 2
                top = (image.size[1] - FEED_SIZE[1]) / 2
                right = left + FEED_SIZE[0]
                bottom = top + FEED_SIZE[1]
                image = image.crop((left, top, right, bottom))

                # Ajustar o tamanho da logo
                scale_factor = 0.2
                new_logo_width = int(FEED_SIZE[0] * scale_factor)
                ...
                margin_top = 5
                margin_right = 5
                position = (FEED_SIZE[0] - new_logo_width - margin_right, margin_top)
